...
def process_matches(
    ...
    Full data-pipeline:
    1) Preprosess
    2) Sikre riktig dtype
    3) Legg på rolling- og statiske features
    """
    # 1) Preprosessering
    df = preprocess_data(df)

    # 2) Dtype-sjekk: gjør om mål, resultater etc. til numerisk
    numeric_cols = ["gf_home", "ga_home", "gf_away", "ga_away", "result_home"]
    df = ensure_numeric(df, numeric_cols)

    # 3) Feature-engineering
    df = add_all_features(df, rolling_stats, rolling_windows)
